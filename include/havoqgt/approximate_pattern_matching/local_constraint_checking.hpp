#ifndef HAVOQGT_APPROXIMATE_PATTERN_MATCHING_LOCAL_CONSTRAINT_CHECKING_HPP_INCLUDED
#define HAVOQGT_APPROXIMATE_PATTERN_MATCHING_LOCAL_CONSTRAINT_CHECKING_HPP_INCLUDED
...
public:
  ...
  BitSet template_neighbors_persistent;
  std::unordered_set<size_t> non_local_constraints;   
...
//              std::cerr << "Error: did not find the expected item in the map." << std::endl; // TODO: is this correct?
      ...
      if (std::get<8>(alg_data) == 0  && std::get<9>(alg_data)) {
        ...
        if (msg_type == 1) {
          ...
          verify_and_update_vertex_state_2(alg_data, vertex_template_vertices);
          return false;        
        }
      } // if
...
//--      return true;
  ...
  template<typename AlgData>
  uint8_t verify_and_update_vertex_state_2(AlgData& alg_data,
    ...
    // verify if vertex heard from a valid parent (i.e., a neighbor)

    bool match_found = false;     
    bool valid_parent_found = false;
    
    for (size_t template_vertex = 0; 
      template_vertex < vertex_template_vertices.size(); template_vertex++) {  
      if (vertex_template_vertices.test(template_vertex)) { // bit position 
        match_found = true;

        // verify if vertex heard from a valid parent (i.e., a neighbor)
        BitSet tmp_bitset_a = pattern_graph.vertex_edges_bitset[template_vertex] & parent_template_vertices_bitset;
        BitSet tmp_bitset_b = pattern_graph.vertex_edges_bitset_optional[template_vertex] & parent_template_vertices_bitset;
        if (!tmp_bitset_a.none() || !tmp_bitset_b.none()) {
        //if (!tmp_bitset_a.none()) {
          valid_parent_found = true;
          break;   
        }    
      }
...
//              } 
    ...
    */
    ...
    if (!match_found || (match_found && !valid_parent_found)) {
      //std::cout << "Ignorign invalid parent." << std::endl; // Test
      return 0; //false;
    }
    ...
    // vertex heard from a valid parent (i.e., a neighbor) 
    // create an entry for this vertex in the vertex_state_map or
    // if exists, update its state  
    ...
    if (find_vertex == std::get<6>(alg_data).end()) {
      ...
      find_vertex->second.template_vertices = vertex_template_vertices; // set template_verticesB       
    }
    ...
    // add valid parent to the vertex_active_edges_map or mark it as active
    auto find_edge = std::get<12>(alg_data)[vertex].find(g->locator_to_label(parent));
    if (find_edge == std::get<12>(alg_data)[vertex].end()) {
      // first LP superstep of the first iteration
      if (std::get<8>(alg_data) == 0  && std::get<9>(alg_data)) {
      ...
      // first LP superstep of the first iteration
      if (std::get<8>(alg_data) == 0  && std::get<9>(alg_data)) {
        find_edge->second = 1; // should never get here // TODO: receiving multiple messages from the same neighbor? 
        //std::cerr << "Error: unexpected item in the map." << std::endl;
        //return 0;  
      } else {
        find_edge->second = 1;
  ...
  template<typename AlgData>
  uint8_t verify_and_update_vertex_state(AlgData& alg_data,
    BitSet& vertex_template_vertices) const {
...
/*    for (size_t i = 0; i < find_vertex->second.template_neighbors.size(); i++) {
    ...
    // add valid parent's template_vertices to the vertex_active_edges_map
    auto find_edge = std::get<12>(alg_data)[vertex].find(g->locator_to_label(parent));
    if (find_edge == std::get<12>(alg_data)[vertex].end()) {
      // first LP superstep of the first iteration
      if (std::get<8>(alg_data) == 0  && std::get<9>(alg_data)) {
  ...
  for (auto& v : vertex_state_map) {
    ...
    for (size_t vertex_pattern_index = 0; // TODO: change the name vertex_pattern_index to template_vertex
      ...
      if (v.second.template_vertices.test(vertex_pattern_index)) {
        ...
        bool has_valid_mandatory_neighbors = false; 
        bool has_valid_optional_neighbors = false;
...
//        }
        tmp_pattern_vertex_edges = pattern_graph.vertex_edges_bitset[vertex_pattern_index];
        ...
        if (pattern_graph.vertex_edges_bitset[vertex_pattern_index].none()) {
          // template vertex with no mandatory edge  
          has_valid_mandatory_neighbors = true;  
        } else {
          // template vertex with mandatory edge(s)
        ...
        // optional edge count  
        if (pattern_graph.vertex_min_optional_edge_count[vertex_pattern_index] > static_cast<int64_t>(0)) {
          BitSet tmp_bitset_b = pattern_graph.vertex_edges_bitset_optional[vertex_pattern_index] & v.second.template_neighbors;
          if (tmp_bitset_b == pattern_graph.vertex_edges_bitset_optional[vertex_pattern_index] && 
            tmp_bitset_b.count() >= static_cast<size_t>(pattern_graph.vertex_min_optional_edge_count[vertex_pattern_index])) {
              has_valid_optional_neighbors = true;   
          }   
	} else {
          has_valid_optional_neighbors = true;   
        }        
...
//        if ((tmp_pattern_vertex_edges == tmp_bitset) && !tmp_bitset.none()) {
        if (has_valid_mandatory_neighbors && has_valid_optional_neighbors) {         
    ...
    if (v.second.template_vertices.none()) {
      ...
      v.second.template_neighbors_persistent =  v.second.template_neighbors;
      assert(!v.second.template_neighbors_persistent.none());
...
template <typename TGraph, typename Vertex, typename Edge, typename VertexData, 
  ...
  typename TemplateVertex, typename VertexStateMapGeneric, 
  typename PatternGraph, typename PatternNonLocalConstraint, typename Boolean,
  typename BitSet>
void local_constraint_checking_iterative (TGraph* g, 
  ...
  size_t global_itr_count,
  bool global_init_step, bool& global_not_finished, 
  ...
  typedef uint8_t VertexIteration; // TODO: dummy, remove 
 
  uint8_t pattern, pattern_indices, vertex_rank, vertex_iteration; // TODO: dummy, remove 
...
#endif //HAVOQGT_APPROXIMATE_PATTERN_MATCHING_LOCAL_CONSTRAINT_CHECKING_HPP_INCLUDED 
