...
class pattern_non_local_constraint {
  public:
    ...
    pattern_non_local_constraint(PatternGraph& pattern_graph, 
    ...
    pattern_non_local_constraint(std::string pattern_input_filename, 
        ...
        if (is_integral_type) {
          input_patterns.push_back(
            std::forward_as_tuple(
              ...
              std::stoull(tokens[5]) // 0 - skip LCC, 1 - invoke LCC if required  
            )
          );
